...
function blocksToHtml(blocks) {
  const rows = blocks.map(b => {
    switch (b.type) {
      case 'heading': {
        ...
        const sz = ['32px', '24px', '20px'][lvl - 1] ?? '24px';
        return `<h${lvl} style="margin:0 0 14px;font-size:${sz};font-weight:700;color:#111;">${b.props.content || ''}</h${lvl}>`;
      }
      case 'text':
        return `<p style="margin:0 0 14px;font-size:15px;line-height:1.65;color:#333;">${(b.props.content || '').replace(/\n/g, '<br>')}</p>`;
      case 'button': {
        const bg = b.props.color || '#4facfe';
        return `<div style="text-align:center;margin:20px 0;"><a href="${b.props.url || '#'}" style="display:inline-block;padding:13px 30px;background:${bg};color:#fff;text-decoration:none;border-radius:6px;font-size:15px;font-weight:600;">${b.props.label || 'Click here'}</a></div>`;
      }
      case 'image':
        return b.props.src
          ? `<img src="${b.props.src}" alt="${b.props.alt || ''}" style="display:block;max-width:100%;margin:12px auto;">`
          : '';
      case 'divider':
        return `<hr style="border:none;border-top:1px solid #e4e8ed;margin:20px 0;">`;
      case 'spacer':
  ...
  return `<div style="max-width:600px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;padding:32px 24px;">\n${rows.join('\n')}\n</div>`;
}
  ...
  return (
    <div className="block-editor">
      <div className="block-editor-row">
        ...
        />
        <input
          type="color" value={props.color || '#4facfe'}
    ...
    <div className="block-editor">
      ...
      {props.src && (
        <img
          ...
          style={{ maxHeight: 80, maxWidth: '100%', marginTop: 6, borderRadius: 4, objectFit: 'cover' }}
...
function DividerPreview() {
  return (
    <div className="block-editor">
      <hr style={{ border: 'none', borderTop: '1px solid #e4e8ed', margin: '4px 0' }} />
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="email-block-card">
        ...
        {block.type === 'divider'  && <DividerPreview />}
  ...
  return (
    <div className="rich-email-editor">
      ...
      <div className="editor-tabs">
        <button
          className={`editor-tab${mode === 'visual' ? ' active' : ''}`}
